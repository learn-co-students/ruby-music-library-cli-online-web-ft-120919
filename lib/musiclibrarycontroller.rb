class MusicLibraryController
  ...
  def library(klass = Song)
    sorted_library = klass.all.collect{ |x| x if x.class == klass }
    sorted_library = sorted_library.delete_if { |x| x == nil }
    sorted_library.uniq
  end
  ...
  def list_artists
    sorted_library = self.library(Artist).sort_by { |x| x.name }
    artists = sorted_library.collect { |x| "#{x.name}"}.uniq
  ...
  def play_song
    ...
    if user_input.positive? && user_input <= self.library.size
